 ...
 * 放大 / 还原 窗口
 *
 * 主窗口是无边框 + 透明窗口，并以固定尺寸创建（maximizable: false）。
 * 实测直接调用系统 maximize() 后，透明窗口的合成表面不会随新尺寸重建，
 * 会出现「窗口变大了但界面不重绘（整窗空白）」的问题；
 * 因此这里改为把窗口铺满当前显示器工作区，并记录放大前的位置用于还原。
 * 这样既不改动 createWindow 的初始窗口配置，也不会触发透明窗口的重绘问题。
 */
...
const toggleMaximize = () => {
  ...
  try {
    browserWindow = BrowserWindow.fromWebContents(getWebContents())
  } catch {
    browserWindow = null
  }
  ...
  // 还原：回到放大前记录的窗口位置
  if (restoreBounds) {
    browserWindow.setBounds(restoreBounds)
    restoreBounds = null
    return
  }

  // 放大：铺满主显示器工作区
  restoreBounds = browserWindow.getBounds()
...
export default () => {
  ...
  mainOn(WIN_MAIN_RENDERER_EVENT_NAME.max_toggle, () => {
    toggleMaximize()
  })
