...
export default {
  ...
  async getList(retryNum = 0) {
    return this.getRawList(retryNum).then(rawList => ({ source: 'tx', list: this.filterList(rawList) }))
  },
  /**
  ...
  async getListWithHot(retryNum = 0) {
    return this.getRawList(retryNum).then(rawList => ({ source: 'tx', list: this.filterListWithHot(rawList) }))
  },
  async getRawList(retryNum = 0) {
    ...
    // console.log(body)
    return body.hotkey.data.vec_hotkey
  },
  filterList(rawList) {
    return rawList.map(item => item.query)
  },
  // hot 为接口 score（搜索热度，用于「热门搜索」右侧数字）
  filterListWithHot(rawList) {
    return rawList.map(item => ({ name: item.query, hot: Number(item.score) || 0 }))
  },
}
