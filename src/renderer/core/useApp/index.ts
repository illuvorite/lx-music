...
export default () => {
  ...
  // 皮肤透明度：解析主题主背景色后写入面板底色（--qm-surface，见 store/utils.applySkinSurface）
  applySkinSurface()
  watch(() => appSetting['theme.skinOpacity'], applySkinSurface)

  // 启动/刷新后按配置恢复主题：URL 注入的主题停留在应用启动时的选择，
  // 之后切换的主题在刷新/重启时需要按配置重新应用，否则会「丢主题」
