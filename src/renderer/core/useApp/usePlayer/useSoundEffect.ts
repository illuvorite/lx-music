...
export default () => {
  ...
  watch(() => appSetting['player.soundEffect.enhance.bass'], (v, old) => {
    console.log('[enhance-debug] bass:', old, '->', v, new Error('trace').stack)
    setBassBoost(v * 0.3)
  })
  watch(() => appSetting['player.soundEffect.enhance.hifi'], (v, old) => {
    console.log('[enhance-debug] hifi:', old, '->', v, new Error('trace').stack)
    setHiFiBoost(v * 0.24)
  })
  watch(() => appSetting['player.soundEffect.enhance.balance'], (v, old) => {
    console.log('[enhance-debug] balance:', old, '->', v, new Error('trace').stack)
    setStereoBalance(v / 50)
  })
  watch(() => appSetting['player.soundEffect.enhance.dynamic'], (v, old) => {
    console.log('[enhance-debug] dynamic:', old, '->', v, new Error('trace').stack)
    setDynamicBoost(v / 50)
  })
  watch(() => appSetting['player.soundEffect.convolution.sendGain'], (v, old) => {
    console.log('[enhance-debug] reverb:', old, '->', v, new Error('trace').stack)
  })
  watch(() => appSetting['player.soundEffect.panner.soundR'], (v, old) => {
    console.log('[enhance-debug] surround:', old, '->', v, new Error('trace').stack)
  })
